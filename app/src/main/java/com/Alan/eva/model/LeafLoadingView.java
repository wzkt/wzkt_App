...
public class LeafLoadingView extends View {
	...
	public LeafLoadingView(Context context, AttributeSet attrs) {
		...
		mLeftMargin = PixelUtils.dipToPx(context, LEFT_MARGIN);
		mRightMargin = PixelUtils.dipToPx(context, RIGHT_MARGIN);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawBitmap(mOuterBitmap, mOuterSrcRect, mOuterDestRect, mBitmapPaint);// 最外层
		// src为你要画的bitmap的区域，dst为你要显示在屏幕上的区域
	...
	private void initPaint() {
		...
		mBitmapPaint.setFilterBitmap(true);// 对位图就行滤波处理

		mWhitePaint = new Paint();
		mWhitePaint.setAntiAlias(true);
		mWhitePaint.setColor(Color.parseColor("#FFC5B9"));

		mOrangePaint = new Paint();
		mOrangePaint.setAntiAlias(true);
		mOrangePaint.setColor(Color.parseColor("#F06548"));
	...
	private void drawProgressAndLeafs(Canvas canvas) {
		...
		// mProgressWidth为进度条的宽度，根据当前进度算出进度条的位置s
		mCurrentProgressPosition = mProgressWidth * mProgress / TOTAL_PROGRESS;
		// 即当前位置在图中所示1范围内
		if (mCurrentProgressPosition < mArcRadius) {
			Log.i(TAG, "mProgress = " + mProgress + "---mCurrentProgressPosition = " + mCurrentProgressPosition
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mArcRadius = (mTotalHeight - 2 * mLeftMargin) / 2;// 圆弧半径
